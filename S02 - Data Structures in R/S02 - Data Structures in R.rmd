---
title: 'Introduction to R for Data Science: Session 02: Introduction to Data Structures in R'
...
The default value for numeric vectors is `0`, while for logical vectors is `FALSE`. You always need to type capital letters to adress logical constants of `FALSE` and `TRUE` in R, respectively, but you can abbreviate logicals as `T` or `F` (capital letters again!)
...
And finally, we also have `mode9()`:

```{r echo = T}
mode(num_vect)
```

The difference between `typeof()` and `mode()` reside on a rather deep level of the structure of R as a programming language. At this point, it is not really relevant for us. To learn more check out [this](https://stackoverflow.com/questions/35445112/what-is-the-difference-between-mode-and-class-in-r) Stack Overflow discussion.
...
Later on, we will learn about `%`, `%>%`, and similarly looking things. NOTE: They do not bear any *semantic* similarity necessarily.
...
+ Chapters 4 to 8 from [R for Data Science, Garrett Grolemund Hadley Wickham](http://r4ds.had.co.nz/)
